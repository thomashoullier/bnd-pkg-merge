...
(defun encode-limited (probs-i L)
  "Compute a compressed representation of length-limited Huffman code lengths
using the boundary package-merge algorithm.
probs-i: Array of integer symbol frequencies of occurence. Strictly above zero.
L: The code length limit. The resulting code lengths will be at most L.
   L must be at least ceil(log(n)/log(2)) included (n>1)."
  ;; Checking the input.
  (let ((n (length probs-i))
        (last-prob 1))
    ...
    ;; Check that the input is sorted in increasing order.
    (loop for prob across probs-i do
      (when (< prob last-prob)
